---
title: "OpenStack: perform consistent snapshots with Qemu Guest Agent"
date: 2015-02-09 16:17:00
slug: openstack-perform-consistent-snapshots-with-qemu-guest-agent
draft: false
categories: ["openstack"]
tags: ["openstack"]
---

![OpenStack: perform consistent snapshots with Qemu Guest Agent](/images/openstack-consistent-snapshot-qemu-agent.jpg)
...
A virtual machine boots with a new virtio device attached pointing to unix socket on the hypervisor.
Inside the virtual machine this socket will appear as a new character device, commonly under `/dev/virtio-ports/`.

A picture is always better:

![QEMU agent freeze and thaw sequence](/images/qemu-agent-freeze-thaw.png)
...
![Iced that guy... To cone a phrase](/images/iced-that-guy.png)

Full reference of the image available [here](https://www.youtube.com/watch?v=No82kn1EfPk) :D.
...
```bash
ubuntu@agent:~$ sudo wget -O /etc/qemu/fsfreeze-hook https://raw.githubusercontent.com/qemu/qemu/master/scripts/qemu-guest-agent/fsfreeze-hook
...
For a more concrete example, have a [look at this work](https://chromium.googlesource.com/external/qemu/+/refs/heads/master/scripts/qemu-guest-agent/fsfreeze-hook.d/mysql-flush.sh.sample) for a database from Chromium.
