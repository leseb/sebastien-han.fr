...
date: 2015-06-23 00:17:00
slug: bootstrap-your-ceph-cluster-in-docker
draft: false
categories: ["containers"]
tags: ["containers"]
---

![Bootstrap your Ceph cluster in Docker](/images/bootstrap-ceph-cluster-docker.jpg)

Almost two years have passed since my first attempt to [run Ceph inside Docker](http://www.sebastien-han.fr/blog/2013/09/19/how-I-barely-got-my-first-ceph-mon-running-in-docker/).
Time has elapsed and I haven't really got the time to resume this work until recently.
For the last couple of months, I have been devoting a third part of my time to contributing on deploying [Ceph in Docker](https://github.com/ceph/ceph-docker).
Before we start, I would like to highlight that nothing of this work would have been possible without the help of [Seán C. McCord](http://www.cycoresys.com).
...
We have a lot of images available on [Docker Hub](https://hub.docker.com).
...
Historically we had (and we still do until we merge this [patch](https://github.com/ceph/ceph-docker/pull/94)) one image per daemon.
So one container image for monitor, osd, mds and radosgw.
This is not really ideal and in practice not needed.
This is why we [worked](https://github.com/ceph/ceph-docker/pull/78) on a single container image called `daemon`.
...
```bash
$ sudo docker run -v /osds/1:/var/lib/ceph/osd/ceph-1 -v /osds/2:/var/lib/ceph/osd/ceph-2
...
A very first step is to use [ceph-ansible](https://github.com/ceph/ceph-ansible) where the logic is already implemented.
